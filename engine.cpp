...
{
	return  mem.Read<UE_UClass*>(this + Offsets.UObject.Class);
}

UE_UObject* UE_UObject::GetOuter()
{
	return  mem.Read<UE_UClass*>(this + Offsets.UObject.Outer);
}

string UE_UObject::GetName()
{
	uint32_t NameId =  mem.Read<uint32_t>(this + Offsets.UObject.Name);
...
{
	return  mem.Read<Tarray>(this + Offsets.UEnum.Names);
}
...
{
	return  mem.Read<uint64_t>(this + Offsets.UFunction.Func);
}
...
{
	return  mem.Read<int>(this + Offsets.UStruct.PropertiesSize);
}

UE_UStruct* UE_UStruct::GetSuper()
{
	return  mem.Read<UE_UStruct*>(this + Offsets.UStruct.SuperStruct);
}

UE_FField* UE_UStruct::GetChildProperties()
{
	return  mem.Read<UE_FField*>(this + Offsets.UStruct.ChildProperties);
}

uint32_t UE_UStruct::GetPropertiesSize()
{
	return  mem.Read<uint32_t>(this + Offsets.UStruct.PropertiesSize);
}

int UE_FField::GetSize()
{
	return  mem.Read<int>(this + Offsets.FProperty.ElementSize);
}

int UE_FField::GetOffset()
{
	return  mem.Read<int>(this + Offsets.FProperty.Offset);
}

string UE_FField::GetName()
{
	uint32_t NameId =  mem.Read<uint32_t>(this + Offsets.FField.Name);
	return NamePoolData->GetName(NameId);
}

string UE_FField::GetType()
{
	uint32_t NameId =  mem.Read<uint32_t>( mem.Read<uint32_t*>(this + Offsets.FField.Class));
...
{
	return  mem.Read<UE_FField*>(this + Offsets.FField.Next);
}

UE_UObject* UE_FField::GetUEObject()
{
	return  mem.Read<UE_UObject*>(this + Offsets.FProperty.Size);
}

string UE_FMapProperty::GetTypeStr()
{
	UE_FField* KeyProp =  mem.Read<UE_FField*>(this + Offsets.FProperty.Size);
	UE_FField* ValueProp =  mem.Read<UE_FField*>(this + Offsets.FProperty.Size + 0x8);
...
{
	if ( mem.Read<uint8_t>(this + Offsets.FProperty.Size + 3) == 0xFF)
		return "bool";
...
{
	UE_UObject* obj =  mem.Read<UE_UObject*>(this + Offsets.FProperty.Size + 8);
...
{
	return "enum class " +  mem.Read<UE_UObject*>(this + Offsets.FProperty.Size + 8)->GetName();
}
...
{
	static File Enum(SaveFileName.EnumName);
	...
	{
		uint32_t Value =  mem.Read<uint32_t>(Names.Data + i * Offsets.UEnum.Size + Offsets.UEnum.Value);
		uint32_t NameId =  mem.Read<uint32_t>(Names.Data + i * Offsets.UEnum.Size + Offsets.UEnum.Name);
		string Name = "    " + NamePoolData->GetName(NameId);
	...
	Enum.fprintf(FullName + ClassName + Type + "\n{\n" + Body + "\n};\n\n\n");
}

void UE_UClass::Generate()
{
	static File Struct(SaveFileName.ClassName);
	...
	Struct.fprintf(FullName + ClassSize + ClassName + Body + "};\n\n\n");
}

void UE_UFunction::Generate()
{
	static File Function(SaveFileName.FunctionName);
	...
	Function.fprintf(FullName + Offset + Ret + FuncName + "(" + Body + ");\n\n\n");
}

void UE_UScriptStruct::Generate()
{
	static File Struct(SaveFileName.StructName);
