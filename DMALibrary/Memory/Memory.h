...
public:
	...
	*/
	...
	uint64_t FindSignature(const char* signature, uint64_t range_start, uint64_t range_end, int PID);
	/**
	...
	* @return true if successful, false if not.
	*/
	bool Read(uintptr_t address, void* buffer, size_t size, bool cache = true) const;
	bool Read(uintptr_t address, void* buffer, size_t size, int pid, bool cache = true) const;
	...
	*/
	template <typename T>
	T Read(void* address, bool cache = true)
	{
		T buffer{ };
	...
	template <typename T>
	T Read(uint64_t address, bool cache = true)
	{
		return Read<T>(reinterpret_cast<void*>(address), cache);
	}
	template<typename T>
	void Read_vec(uintptr_t address, size_t size, std::vector<T>* vec) {

		if (!size)
			return;
		if (sizeof(T) * size > (300000 * 0x20)) {
			printf("Error: Requested size exceeds maximum allowed size!\n");
			return;
		}
		vec->resize(size);
		Read(address, vec->data(), sizeof(T) * size);
	...
	*/
	template <typename T>
	T Read(void* address, int pid, bool cache = true)
	{
		T buffer{ };
		memset(&buffer, 0, sizeof(T));
	...
	template <typename T>
	T Read(uint64_t address, int pid, bool cache = true)
	{
