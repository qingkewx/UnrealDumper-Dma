...
{
	LPCSTR args[] = { (LPCSTR)"", (LPCSTR)"-device", (LPCSTR)"fpga://algo=0", (LPCSTR)"", (LPCSTR)"" };
	int argc = 3;
	if (debug)
	{
		args[argc++] = (LPCSTR)"-v";
...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	{
	reinit:
		LPCSTR args[] = { (LPCSTR)"", (LPCSTR)"-device", (LPCSTR)"fpga://algo=0", (LPCSTR)"", (LPCSTR)"", (LPCSTR)"", (LPCSTR)"" };
		DWORD argc = 3;
		if (debug)
		{
			args[argc++] = (LPCSTR)"-v";
		...
		if (memMap)
		{
			...
			else
			{
				...
				args[argc++] = (LPCSTR)"-memmap";
				args[argc++] = (LPCSTR)path.c_str();
			}
		}
		args[argc++] = (LPCSTR)"-norefresh";
	...
	{
		LOG("Failed get current_process\n");
		return false;
	}
	...
	{
		LOG("Failed get base_address\n");
		return false;
	}
	BaseAddr = this->current_process.base_address;
	BasePid = this->current_process.PID;
...
bool Memory::FixCr3()
{
	...
	while (true)
	{
		...
		//std::cout << "[-] Percentage:" << atoi((LPSTR)bytes) << std::endl;
	...
	for (size_t i = 0; i < possible_dtbs.size(); i++)
	{
		...
		if (result)
		{
			...
			VMMDLL_ConfigSet(this->vHandle, VMMDLL_OPT_PROCESS_DTB | this->current_process.PID, 666);*/
			LOG("[+] Patched DTB\n");
			return true;
		}
	}
